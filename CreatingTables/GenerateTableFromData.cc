...
void GenerateTableFromData(bool likeSign = false, bool kInt7 = false, string dataDir = "" , string tableDir = "")
{
  ...
  // Directories where files are saved
  string dataDir2 = "/afs/cern.ch/user/e/ejobst/Hypertriton_analysis/Data";
  string tableDir2 = "/afs/cern.ch/user/e/ejobst/Hypertriton_analysis/Data";

  string pass = "3";
  // string otf = getenv("HYPERML_OTF");
  
  string passString = "_pass" + pass + "_test2.root"; //Changed something here
  ...
  string inFileName = "HyperTritonTree_18r";
  string inFileArg = dataDir2 + "/" + inFileName + ".root";//Changed something here

  string outFileName = "DataTable_18";
  string outFileArg = tableDir2 + "/" + outFileName + kintstring + lsString + passString;
